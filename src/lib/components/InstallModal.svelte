<script lang="ts">
  ...
  async function toggle(tool: Tool, target: string | null) {
    ...
    try {
      ...
      if (fail === 0) toast.success(i18n.t("install.toastInstalled", { count: i18n.number(ok), tool: install.toolLabel(tool), where }));
      else toast.error(i18n.t("install.toastInstallFail", { tool: install.toolLabel(tool), ok: i18n.number(ok), fail: i18n.number(fail) }));
    } finally {
      busy = null;
    }
  }
  ...
  async function remove(tool: Tool, target: string | null, rs: InstalledAgent[]) {
    ...
    try {
      ...
      if (fail === 0) toast.success(i18n.t("install.toastRemoved", { count: i18n.number(ok), tool: install.toolLabel(tool) }));
      else toast.error(i18n.t("install.toastRemoveFail", { tool: install.toolLabel(tool), ok: i18n.number(ok), fail: i18n.number(fail) }));
    } finally {
      busy = null;
    }
  }
...
<Modal open {title} size="wide" onClose={onClose}>
  <p class="sub">{i18n.t("install.sub", { count: i18n.number(total), agents: i18n.agents(total) })}</p>
      ...
      {#each cols as t (t.id)}
        {#if applicable(row, t)}
          ...
          <button
            ...
            title={`${t.label} · ${row.kind === "global" ? i18n.t("common.global") : row.label}`}
            aria-label={`${cov.all ? i18n.t("install.removeFrom") : i18n.t("install.installInto")} ${t.label} ${row.kind === "global" ? i18n.t("install.globally") : i18n.t("install.inProject", { project: row.label })}`}
...
{#if confirm}
  ...
  <DestructiveConfirm
    open
    title={i18n.t("install.deleteTitle", { count: i18n.number(n), files: i18n.files(n), label })}
    confirmLabel={i18n.t("install.deleteConfirm", { count: i18n.number(n) })}
    onConfirm={confirmRemove}
    onCancel={() => (confirm = null)}
  >
    <p>{i18n.t("install.deleteBody", { count: i18n.number(n), files: i18n.files(n) })}</p>
  </DestructiveConfirm>
{/if}
