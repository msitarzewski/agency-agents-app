<script lang="ts">
  ...
  async function choose(fn: () => Promise<unknown>, ok: string) {
    try {
      ...
      toast.error(i18n.t("firstRun.couldNotSet"), String(e));
    }
  }
...
<div class="scrim">
  <div class="box" role="dialog" aria-modal="true" aria-label={i18n.t("firstRun.aria")}>
    <header>
      <h1>{i18n.t("firstRun.title")}</h1>
      <p class="lede">
        {i18n.t("firstRun.lede")}
      </p>
    </header>
    ...
    <div class="cards">
      ...
      <div class="card" class:open={expanded === "clone"}>
        <button class="card-head" onclick={() => (expanded = expanded === "clone" ? null : "clone")}>
          <FolderGit2 size={22} />
          <div class="ct">
            <span class="t">{i18n.t("firstRun.useOwnClone")}</span>
            <span class="d">{i18n.t("firstRun.useOwnCloneDesc")}</span>
          </div>
        </button>
          ...
          <div class="card-body">
                    ...
                    <button class="cand" disabled={catalog.busy} onclick={() => useCandidate(c)}>
                      <div class="cand-main">
                        ...
                        <span class="cand-meta">
                          {i18n.t("firstRun.candidateMeta", { count: i18n.number(c.agentCount), agents: i18n.agents(c.agentCount), git: c.hasGit ? " · git" : "", kind: c.kind === "managed" ? "~/.agency-agents" : i18n.t("firstRun.found") })}
                        </span>
                      </div>
                      <Check size={15} />
                    </button>
            ...
            <div class="row-actions">
              <button class="ghost" disabled={catalog.scanning} onclick={() => catalog.detect(true)}>
                <Search size={14} /><span>{catalog.scanning ? i18n.t("firstRun.searching") : i18n.t("catalog.findAgency")}</span>
              </button>
